...
def qml_circuit_qiskit_01(n_wires=2, n_layers=1):
    ...
    for j in range(n_layers):
        for k in range(n_wires):
            ...
            ansatz.cry(Parameter('{}cw_{}'.format(str(j), str(k))), k, (k+1) % n_wires)
        if j != n_layers-1:
            ansatz.barrier()
...
def qml_circuit_qiskit_02(n_wires=2, n_layers=1):
    ...
    for j in range(n_layers):
        ...
        for l in range(n_wires):
            ansatz.cry(Parameter('{}cw_{}'.format(str(j), str(l))), l, (l+1) % n_wires)
        if j != n_layers-1:
            ansatz.barrier()
...
def qml_circuit_qiskit_04(n_wires=2, n_layers=1):
    ...
    for j in range(n_layers):
        for k in range(n_wires):
            ansatz.rx(Parameter('{}w_{{x{}}}'.format(str(j), str(k))), k)
            ansatz.ry(Parameter('{}w_{{y{}}}'.format(str(j), str(k))), k)
            ansatz.rz(Parameter('{}w_{{z{}}}'.format(str(j), str(k))), k)
        if j != n_layers-1:
            ansatz.barrier()
...
# circuit = qml_circuit_qiskit_05(N_WIRES, 1)
# print("\nQC5")
# print(circuit.draw(vertical_compression='high', fold=-1, scale=0.5))
# print(circuit.draw(output='latex_source', vertical_compression='high', fold=-1, scale=0.5))
